...
//            [self moveItemAtIndexPath:_originalIndexPath toIndexPath:_moveIndexPath];
            [self moveSection:_originalIndexPath.section toSection:_moveIndexPath.section];
...
- (void)xwp_updateDataSource{
    ...
    if (dataTypeCheck) {
        for (int i = 0; i < temp.count; i ++) {
            if (!self.isDragSection) {
                [temp replaceObjectAtIndex:i withObject:[temp[i] mutableCopy]];
            } else {
                [temp replaceObjectAtIndex:i withObject:temp[i] ];
            }
    ...
    if (_moveIndexPath.section == _originalIndexPath.section) {
        ...
        if (!self.isDragSection) {
            ...
            [temp exchangeObjectAtIndex:_originalIndexPath.section withObjectAtIndex:_moveIndexPath.section];
        }
